...
- `server/routes/gameRoutes.js`:
  - **新增階段限制**：禁止在「等待中 (waiting)」階段進行等級提升，確保遊戲平衡。
  - **重置邏輯強化**：在遊戲正式開始 (`/start`) 時，強制重置所有玩家的升級標記與準備狀態，徹底解決等待室階段殘留狀態導致的 Bug。
- `client/src/App.vue`:
  - **按鈕互動優化**：前端同步阻斷等待階段的升級請求，按鈕會顯示「遊戲開始後才可升級」並不允許點擊。
  - **版本號同步**：統一各處顯示的版本號至 v1.9.10。
...
- `client/src/App.vue`:
  - **同步前端按鈕邏輯**：修正升級按鈕的狀態檢查，從「需 > 30 HP」改為「只要 > 消耗 HP」即可。
  - **按鈕狀態顯示**：新增「本回合已升級」的提示與鎖定狀態。
- `client/src/components/AdminPanel.vue`:
  - **簡化確認文字**：移除刪除戰局時「所有相關資料將被清除」的冗餘提示。
