# 森林進化遊戲 - 專案快照 (PROJECT SNAPSHOT)

> **生成時間**: 2026-02-02  
> **專案版本**: 1.9.1 (Production Stable)  
> **目的**: 記錄專案架構、核心邏輯位置與關鍵檔案,供開發者快速理解專案全貌

---

## 📁 專案資料夾結構

```text
forest-evolution-game/
├── client/                          # 前端應用 (Vue 3 + Vite)
│   ├── public/
│   │   ├── admin.html              # 管理員頁面 (靜態)
│   │   └── vite.svg
│   ├── src/
│   │   ├── components/             # Vue 元件
│   │   │   ├── AdminPanel.vue      # 管理員控制面板
│   │   │   ├── AuctionModal.vue    # 競標彈窗
│   │   │   ├── GameRules.vue       # 遊戲規則顯示
│   │   │   ├── PlayerDashboard.vue # 玩家儀表板
│   │   │   └── SkillSelectionModal.vue # 技能選擇彈窗
│   │   ├── utils/
│   │   │   └── gameHelpers.js      # 遊戲輔助函式
│   │   ├── App.vue                 # 主應用元件 (核心 UI 邏輯)
│   │   ├── main.js                 # Vue 應用入口
│   │   ├── socketService.js        # Socket.IO 客戶端服務
│   │   └── style.css               # 全域樣式
│   ├── index.html                  # HTML 入口
│   ├── vite.config.js              # Vite 設定
│   └── package.json                # 前端依賴
│
├── server/                          # 後端應用 (Node.js + Express + Socket.IO)
│   ├── config/
│   │   └── gameConstants.js        # 遊戲常數 (技能、等級、屬性)
│   ├── models/
│   │   ├── gameModel.js            # 遊戲資料模型 (MongoDB Schema)
│   │   └── playerModel.js          # 玩家資料模型 (MongoDB Schema)
│   ├── routes/
│   │   └── gameRoutes.js           # API 路由定義
│   ├── services/
│   │   ├── gameService.js          # 核心遊戲邏輯服務
│   │   └── aiService.js            # AI 玩家邏輯 (決策大腦)
│   ├── tests/
│   │   └── full-round-test.js      # 完整回合測試
│   ├── server.js                   # 伺服器入口
│   └── package.json                # 後端依賴
│
├── tests/                           # 測試腳本
│   ├── full_game_simulation_8p.js  # 8 人遊戲模擬
│   ├── test_battle_logic_standalone.js # 戰鬥邏輯測試
│   └── test_distribution_logic.js  # 屬性分配測試
│
├── GAME_RULES.md                    # 遊戲規則文件
├── modification_history.md          # 修改歷史紀錄
├── render.yaml                      # Render 部署設定
├── package.json                     # 根目錄依賴 (整合用)
└── .env                             # 環境變數 (MongoDB URI 等)
└── .env                             # 環境變數 (MongoDB URI 等)

---

## 🤖 AI 智能體系 (New in v1.6.0)

**檔案**: `server/services/aiService.js`

**功能**:
- **自動補位**: 當真人玩家不足時，管理員可加入 AI 玩家。
- **性格分類**:
  - `aggressive` (攻擊型): 優先攻擊殘血，競標積極。
  - `defensive` (防禦型): 保守留 HP，競標防禦技。
  - `smart` (聰明型): 計算屬性相剋，理性競標。
  - `clumsy` (笨拙型): 隨機行為。
- **決策與升級**: 討論階段會根據血量與性格自動判斷是否 `levelup`。

---

## 🎯 核心邏輯位置

### 1️⃣ **屬性相剋系統**

**檔案**: `server/services/gameService.js`  
**函式**: `handleSingleAttack()` (第 226-350 行)

```javascript
// 核心邏輯位置 (第 258-296 行)
const attributeRules = { '木': '水', '水': '火', '火': '木' };

// 判定邏輯
if (attacker.attribute === target.attribute) 
    attackSuccess = attacker.level > target.level;
else if (attacker.attribute === '雷' || target.attribute === '雷') 
    attackSuccess = true;
else if (attributeRules[attacker.attribute] === target.attribute) 
    attackSuccess = true;
else if (attributeRules[target.attribute] === attacker.attribute) 
    attackSuccess = false;
```

**相剋規則**:

- 木剋水、水剋火、火剋木
- 雷屬性特殊: 打人必中、被打必中、雷打雷看等級

---

### 2️⃣ **傷害計算系統**

**檔案**: `server/services/gameService.js`  
**函式**: `handleSingleAttack()` 內的 `damageCalculator()` (第 298-302 行)

```javascript
const damageCalculator = (winner, loser) => {
    const effectiveDefense = winner.skills.includes('獠牙') ? 0 : loser.defense;
    const adrenalineBonus = (winner.skills.includes('腎上腺素') && winner.hp < 10) ? 3 : 0;
    return Math.max(1, winner.attack + (roundBonus[game.currentRound] || 0) + adrenalineBonus - effectiveDefense);
};
```

**傷害公式**:

```text
傷害 = Max(1, 攻擊力 + 回合加成 + 技能加成 - 防禦力)
```

**回合加成** (第 288 行):

- 第 1 回合: +3
- 第 2 回合: +4
- 第 3 回合: +5
- 第 4 回合: +7

**技能影響**:

- `獠牙`: 無視防禦力
- `腎上腺素`: HP < 10 時攻擊力 +3
- `適者生存`: 攻擊成功後攻擊力 +2
- `尖刺`: 反彈一半傷害
- `嗜血`: 攻擊成功恢復 2 HP

---

### 3️⃣ **競標系統**

**檔案**: `server/services/gameService.js`  
**核心函式**:

| 函式名稱 | 行數 | 功能 |
| :--- | :--- | :--- |
| `startAuctionForSkill()` | 113-135 | 開始單一技能競標 (5秒倒數) |
| `transitionToActive()` | 87-111 | 轉換為競標進行中 (30秒) |
| `settleSkillAuction()` | 137-169 | 結算競標結果 |
| `finalizeAuctionPhase()` | 171-224 | 完成所有競標,進入下回合 |

**競標流程**:

1. 管理員開始攻擊階段 → 觸發競標
2. 每個技能依序競標 (5秒準備 + 30秒競標)
3. 價高者得,扣除 HP 並獲得技能
4. 所有技能競標完畢 → 進入下一回合討論階段

**出價規則** (`server/routes/gameRoutes.js` 第 297-336 行):

- 必須保留至少 5 HP
- 重複出價會將原出價加回可用預算
- 最後 10 秒內出價會自動加時 10 秒

---

## 🛠️ 技術棧

### **前端**

- **框架**: Vue 3 (Composition API)
- **建置工具**: Vite (rolldown-vite@7.1.14)
- **即時通訊**: Socket.IO Client 4.8.1
- **HTTP 請求**: Axios 1.12.2
- **樣式**: Vanilla CSS (含動畫、響應式設計)

### **後端**

- **執行環境**: Node.js
- **框架**: Express 5.1.0
- **即時通訊**: Socket.IO 4.8.1
- **資料庫**: MongoDB (Mongoose 8.19.1)
- **環境變數**: dotenv 17.2.3
- **跨域**: CORS 2.8.5

### **部署**

- **平台**: Render (參考 `render.yaml`)
- **資料庫**: MongoDB Atlas

---

## ⚠️ 核心檔案清單 (絕對不能隨便大改)

### 1. `server/services/gameService.js` (550 行)

**重要性**: ⭐⭐⭐⭐⭐  
**原因**: 包含所有核心遊戲邏輯

- 屬性相剋判定
- 傷害計算
- 技能效果實作 (15+ 種技能)
- 競標流程控制
- 攻擊流程 (含獅子王手下攻擊)
- 傷害連結 (同病相憐)

**修改建議**: 任何改動都要先寫測試,確保不破壞現有邏輯

---

### 1.5 `server/services/aiService.js` (核心 AI)

**重要性**: ⭐⭐⭐⭐  
**原因**: 控制 AI 玩家的所有行為

- 屬性/技能決策
- 競標加價邏輯
- 自動升級判定

---

### 2. `server/config/gameConstants.js` (49 行)

**重要性**: ⭐⭐⭐⭐⭐  
**原因**: 定義遊戲平衡性的核心數值

- 技能清單與描述 (按回合分類)
- 等級素質 (攻擊/防禦)
- 升級消耗
- 初始 HP

**修改建議**: 改動數值前要先評估對遊戲平衡的影響

---

### 3. `client/src/App.vue` (1870 行)

**重要性**: ⭐⭐⭐⭐  
**原因**: 前端核心邏輯與 UI

- 遊戲狀態管理
- Socket.IO 事件處理
- 技能使用邏輯
- 競標介面
- 競標介面
- 玩家互動 UI
- **管理員安全**: 密碼鎖 (3377) 與 ViewMode 控制

**修改建議**: UI 改動可以,但要小心不要破壞狀態同步邏輯

---

### 4. `server/models/gameModel.js` & `playerModel.js`

**重要性**: ⭐⭐⭐⭐  
**原因**: 資料庫 Schema 定義

- 遊戲狀態結構
- 玩家屬性結構
- 競標狀態
- 回合統計

**修改建議**: Schema 改動會影響現有資料,需要資料遷移腳本

---

### 5. `server/routes/gameRoutes.js` (455 行)

**重要性**: ⭐⭐⭐⭐  
**原因**: API 端點定義

- 遊戲建立/加入/開始
- 攻擊/競標/技能使用
- 管理員功能 (踢人、更新玩家、加入 AI)
- 屬性偵查
- 控制台指令 (強制跳過、強制結束)

**修改建議**: API 改動要同步更新前端呼叫

---

## 📊 資料流向

```
使用者操作 (App.vue)
    ↓
Socket.IO / HTTP API (socketService.js / axios)
    ↓
後端路由 (gameRoutes.js)
    ↓
遊戲服務層 (gameService.js)
    ↓
資料庫模型 (gameModel.js / playerModel.js)
    ↓
MongoDB Atlas
    ↓
Socket.IO 廣播 (broadcastGameState)
    ↓
所有客戶端更新 UI
```

---

## 🔧 常用指令

### 開發環境

```bash
# 安裝依賴
npm run build

# 啟動後端 (開發)
cd server && npm run start

# 啟動前端 (開發)
cd client && npm run dev
```

### 生產環境

```bash
# 建置前端
cd client && npm run build

# 啟動伺服器 (會自動服務前端靜態檔案)
npm start
```

---

## 📝 重要設定檔

| 檔案 | 用途 |
| :--- | :--- |
| `.env` | MongoDB 連線字串、環境變數 |
| `render.yaml` | Render 部署設定 |
| `client/vite.config.js` | Vite 建置設定 |
| `GAME_RULES.md` | 完整遊戲規則文件 |
| `modification_history.md` | 版本修改紀錄 |

---

## 🎮 遊戲核心機制總結

1. **屬性系統**: 木/水/火/雷,相剋關係決定攻擊成敗
2. **等級系統**: Lv0-3,消耗 HP 升級,提升攻擊/防禦
3. **回合制**: 討論階段 → 攻擊階段 → 競標階段 → 下一回合
4. **技能系統**: 15+ 種技能,分為被動/主動/一次性
5. **競標系統**: 盲標,價高者得,扣除 HP
6. **勝利條件**: 最後存活或血量最高者

---

## 🚨 已知限制與注意事項

1. **前三回合保護**: 每位玩家每回合只能被攻擊一次
2. **反擊限制**: 無法攻擊本回合已攻擊過你的玩家
3. **獅子王限制**: 場上僅剩 2 人時無法指定手下
4. **競標保留**: 必須保留至少 5 HP 才能出價
5. **屬性分配**: 木/水/火 平均分配,多餘名額給雷

---

**文件結束** - 如需更新此文件,請同步更新 `modification_history.md`
